...
export class LearningService {
  ...
  /**
   * Busca patrones de conversación relevantes
   */
  static async buscarPatronRelevante(mensaje: string): Promise<PatronConversacion | null> {
    try {
      ...
      console.error('Error buscando patrón:', error)
      return null
    }
  }

  /**
   * Registra feedback de una conversación
   */
  static async registrarFeedback(feedback: FeedbackConversacion): Promise<void> {
    try {
      ...
      // Si requirió corrección, aprender de ello
  ...
  static async detectarIntencion(mensaje: string): Promise<string> {
    ...
    if (
      ...
      /\b(para|el|a las|mañana|hoy|viernes|lunes|martes|miércoles|jueves|sábado|domingo)\b/i.test(
        mensajeLower
      )
    ) {
      return 'agendar'
    ...
    if (/\b(cuánto|precio|costo|valor|información|qué es|cómo)\b/i.test(mensajeLower)) {
      return 'consultar'
    }
    ...
    // Patrones de saludo
    if (/\b(hola|buenos días|buenas tardes|buenas noches|hey)\b/i.test(mensajeLower)) {
      return 'saludo'
    }
  ...
  static async aprenderNuevaPalabra(vocabulario: VocabularioParaguayo): Promise<void> {
    try {
      ...
      console.log(`✅ Aprendida nueva palabra: ${vocabulario.palabra_o_frase}`)
    } catch (error) {
  ...
  /**
   * Registra un nuevo patrón de conversación
   */
  static async aprenderNuevoPatron(patron: PatronConversacion): Promise<void> {
    try {
      ...
      console.log(`✅ Aprendido nuevo patrón: ${patron.categoria}`)
    } catch (error) {
      console.error('Error aprendiendo nuevo patrón:', error)
    }
  }
