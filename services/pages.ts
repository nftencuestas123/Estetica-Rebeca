...
export async function getAllPages(): Promise<Page[]> {
  ...
  if (error) {
    ...
    throw new Error('No se pudieron cargar las páginas')
  }
...
/**
 * Obtener la página root activa (la que se muestra en /)
 */
export async function getRootPage(): Promise<Page | null> {
  ...
  if (error) {
    console.error('Error al cargar página root:', error)
    return null
  }
...
/**
 * Establecer una página como root (página principal)
 */
export async function setRootPage(pageId: string): Promise<void> {
  // El trigger en la BD se encargará de desactivar las demás
  ...
  if (error) {
    console.error('Error al establecer página root:', error)
    throw new Error('No se pudo establecer la página como principal')
  }
}

/**
 * Activar/desactivar una página
 */
export async function togglePageActive(pageId: string, isActive: boolean): Promise<void> {
  ...
  if (error) {
    console.error('Error al cambiar estado de página:', error)
    throw new Error('No se pudo cambiar el estado de la página')
  }
}

/**
 * Incrementar vistas de una página
 */
...
/**
 * Obtener estadísticas generales de páginas
 */
...
/**
 * Obtener una página por su template_key
 */
